...
def loadDataLabelSequence(dir_name, batch_size, delay=DELAY):
    ...
    data_dely = np.zeros([batch_size, epoch_size, FRAME_COUNT, 2 * delay], dtype=np.float32)
    for i in range(batch_size):
        i_dely = i * epoch_size * FRAME_COUNT
        for j in range(epoch_size):
            j_dely = j * FRAME_COUNT
            for k in range(FRAME_COUNT):
                for l in range(delay):
                    data_dely[i, j, k, l * 2] = datas[i_dely + j_dely + k + l + 1, 0]
                    data_dely[i, j, k, l * 2 + 1] = datas[i_dely + j_dely + k + l + 1, 1]


    return _datas, _labels, data_dely