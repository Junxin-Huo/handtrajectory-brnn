...
from loader import loadDataLabelSequence, FRAME_COUNT, DELAY
...
def main(argv=None):
    ...
    data, label, data_dely = loadDataLabelSequence(DATADIR, BATCH_SIZE)
    ...
    x_delay = tf.placeholder(tf.float32, shape=[BATCH_SIZE, 1, DELAY * 2], name='data_delay')
    keep_prob = tf.placeholder(tf.float32, name='prob')

    train_prediction, initial_state, final_state = inference(x, keep_prob, BATCH_SIZE, x_delay)
    ...
    with tf.Session() as sess:
        ...
        for step in range(epoch_size):
            ...
            batch_data_dely = np.reshape(data_dely[:, step, :, :], [BATCH_SIZE, FRAME_COUNT, DELAY * 2])
            frame_length = batch_data.shape[1]
            ls_sub = []
            for j in range(frame_length):
                data_mini = batch_data[:, j, :]
                data_mini_dely = batch_data_dely[:, j, :]
                feed_dict = {x: np.reshape(data_mini, [BATCH_SIZE, 1, 2]),
                             x_delay: np.reshape(data_mini_dely, [BATCH_SIZE, 1, DELAY * 2]),
                             keep_prob: 1.0}
