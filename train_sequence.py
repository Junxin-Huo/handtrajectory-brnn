...
def main(argv=None):
    with tf.Graph().as_default():
        ...
        x_delay = tf.placeholder(tf.float32, shape=[BATCH_SIZE, FRAME_COUNT, DELAY * 2], name='data_delay')
        ...
        with tf.Session() as sess:
            ...
            for step in range(NUM_EPOCHS * epoch_size):
                ...
                batch_data_dely = np.reshape(data_dely[:, offset, :, :], [BATCH_SIZE, FRAME_COUNT, DELAY * 2])
                batch_labels = np.reshape(label[:, offset, :], [BATCH_SIZE, FRAME_COUNT])

                # Train RNN net.
                feed_dict = {x: batch_data,
                             x_delay: batch_data_dely,
                             y: batch_labels,
                             keep_prob: KEEP_PROB}
